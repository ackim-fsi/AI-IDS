...
class DataSequence(keras.utils.Sequence):
	...
	def _load_cache(self, label, idx):
		...
		self._cache_dict[label] = np.concatenate(tuple([self._cache_dict[label]] + list(np.load(x) for x in temp_array_filename_list)))
		gc.collect()
...
def get_session(gpu_share=0.2, threads=2):
	...
	config = tf.ConfigProto(inter_op_parallelism_threads=threads, intra_op_parallelism_threads=threads, gpu_options=gpu_options)
...
def initialize_hyperparameters(parameter_dict):
	...
	if parameter_dict is None:
		# Default Values
		parameter_tuple = (3, 12, 4, 6, 14, 4, 8, 0.5, 0.1)
	else:
		parameter_tuple = (
...
def search_model(datafile_dict, test_file_list, x_dim):
	...
	with open('results.csvx', 'w') as result_file:
		...
		for conv_depth in [3]:
			for conv_filter in [4,6,8,10,12]:
				for conv_kernel_width in [4, 3]:
					for conv_pool in [2, 3, 4, 5, 6]:
						for lstm_units in [16,14,12,8]:
							for dense_depth in [1,2,3,4,5,6,7,8]:
								for dense_units in [16,12,8]:
...
if __name__ == '__main__':
	...
	model_category = "INV-SQL"
	model_type = "CDRNN-" + pad_string
	...
	sql_files = list(os.path.join(npy_dir, f)
					for f in os.listdir(npy_dir)
					if "_sql_" + pad_string in f
	...
	train_model(model, {'payload': payload_files, 'sql': sql_files, 'label': label_files},
