...
if __name__ == '__main__':
    ...
    app_files = list(os.path.join(npy_dir, f)
                     for f in os.listdir(npy_dir)
                     if "_app_" + pad_string in f
    ...
    #train_model(model, {'payload': payload_files, 'app': app_files, 'label': label_files}, x_dim=1000, save_model=True, model_name=model_path, verbose=0)
    train_model(model, {'payload': payload_files, 'app': app_files, 'label': label_files}, x_dim=1000, save_model=True, model_name='INV-APP-CRDNN.h5', verbose=0)
	
	
	#test_model(model, test_file_list, x_dim=1000)
		
		
	#search_model({'payload': payload_files, 'app': app_files, 'label': label_files}, test_file_list, x_dim=1000)
